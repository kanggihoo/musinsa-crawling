...
try:
    for main_key in CATEGORY.keys():
        ...
        for sub_key in CATEGORY[main_key].keys():
            ...
            crawled_data = crawl_product_list(crawler, num_scrolls=1000, **{"category_main": CATEGORY_MAIN[main_key],"category_sub": CATEGORY[main_key][sub_key],"gender": GENDER["남성"]})
            
            # 소분류 데이터를 바로 저장
            print(f"crawling {main_key} , {sub_key} 완료. 데이터 저장 중...")
            sub_category_df = pd.DataFrame(crawled_data , dtype="object")
            save_dataframe_to_csv(
                sub_category_df, 
                SAVE_DIR / f"musinsa_product_summary_{main_key}_{sub_key}_.csv", 
                # f"./data/test_summary_{main_key}_{sub_key}_.csv",
            ...
            # 파일 경로 저장
            main_category_data.append( str(SAVE_DIR / f"musinsa_product_summary_{main_key}_{sub_key}_.csv"))
            # main_category_data.append(f"test_summary_{main_key}_{sub_key}_.csv")
        ...
        save_dataframe_to_csv(
            main_category_df, 
            SAVE_DIR / f"musinsa_product_summary_{main_key}_all_.csv",
            # f"test_summary_{main_key}_all_.csv",
            index_column="product_id"
        )
    ...
    final_dfs = []
    for main_key in CATEGORY.keys():
        temp_df = pd.read_csv(str(SAVE_DIR / f"musinsa_product_summary_{main_key}_all_.csv"))
        final_dfs.append(temp_df)
        del temp_df  # 메모리 해제
    
    final_df = pd.concat(final_dfs, ignore_index=True)
    save_dataframe_to_csv(final_df, SAVE_DIR / "musinsa_product_summary_final_.csv", index_column="product_id")
